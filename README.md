...
The extension expects a configuration object named `config` (either through a JSON or a YAML configuration file passed to the binder) that allows one to define how requests and responses are extracted from messages or built.

Contents of this configuration object are detailed in the following sections.

### General configuration keys

#### client-id

This key allows one to specify the name of the MQTT client used to communicate with the broker.

*Default value*: a random name generated by the MQTT library
...
This key allows one to specify a username to authenticate to the MQTT broker. If it is unset, no authentication is done.

#### broker-pwd

This key allows one to specify a password to authenticate to the MQTT broker.
